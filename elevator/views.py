...
class ElevatorViewSet(
    ...
    def get_direction(self, request, *args, **kwargs):
        ...
        current_request = elevator.elevator_requests.filter(status="PROCESSING").last()
    ...
    def get_next_destination_floor(self, request, *args, **kwargs):
        ...
        current_request = ElevatorRequest.objects.filter(
            status="PROCESSING", elevator=elevator
        ).first()
    ...
    def get(self, request, *args, **kwargs):
        ...
        for elevator_system in ElevatorSystem.objects.all():
            ...
            busy_elevators = Elevator.objects.filter(
                status="busy", elevator_system=elevator_system
            )
            ...
            for elevator in busy_elevators:
                ...
                remaining_time = time_increment - elevator_service.service_elevator(
                    elevator, time_increment
                )
                while remaining_time > 0:
                    ...
                    remaining_time -= elevator_service.service_elevator(
                        elevator, remaining_time
                    )
            ...
            for elevator_request in pending_requests:
                ...
                available_elevator = elevator_service.service_elevator_request(
                    elevator_request
                )
